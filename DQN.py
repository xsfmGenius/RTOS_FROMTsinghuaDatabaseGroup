...
class ENV(object):
    ...
    def reward(self,):
        if self.sel.total+1 == len(self.sel.from_table_list):
            ...
            print("now",time.time()*1000)
            return self.sel.plan2Cost()/self.sql.getDPlantecy(), True
        else:
            return 0,False
...
class DQN:
    def __init__(self,policy_net,target_net,db_info,pgrunner,device):
        ...
        self.optimizer = optim.Adam(policy_net.parameters(),lr = 1e-5   ,betas=(0.9,0.999))
    ...
    def select_action(self, env, need_random = True):
        ...
        # print("sample > eps_threshold",sample > eps_threshold)
        if sample > eps_threshold:
            return action_batch,action_list[torch.argmin(action_batch,dim = 1)[0]][1],[x[1] for x in action_list]
        else:
            randomint=random.randint(0,len(action_list)-1)
            # print("randomint",randomint)
            return action_batch,action_list[randomint][1],[x[1] for x in action_list]
    ...
    def validate(self,val_list, tryTimes = 1):
        ...
        for sql in val_list:
            ...
            for t in count():
                ...
                if done:
                    this_cost=env.sel.plan2Cost()
                    print("thisorder", action_this_epi)
                    print("thisreward", reward)
                    print("thislatency", this_cost)
                    my_cost+=this_cost
        ...
        # logger.info("MRC %s GMRL %s SMRC %s", sum(rewards) / lr, e ** (mes / lr), my_cost / DP_cost)
