...
# file_handler = logging.FileHandler('LatencyTraining.log')
...
def split_queries_by_filename(job_queries):
    test = []
    train = []
    ...
    for sql in sql_list:
        ...
        for t in count():
            ...
            if done:
                ...
                mes += log(reward*pg_cost) - log(pg_cost)
...
def train(trainSet,validateSet):
    ...
    best_val_value = float('inf')  # 初始设为正无穷，记录最小val_value
    best_epoch = 0  # 记录最小val_value对应的Epoch
    ...
    for i_episode in range(0,10000):
        ...
        print(i_episode)
        print(sqlt.filename)
        # logger.info(i_episode)
        # logger.info(sqlt.filename)
        pg_cost = sqlt.getDPlantecy()  ##1A:(mc,ct)(mi_idx,it)(mi_idx,mc)(mc,t)
        ...
        for t in count():
            ...
            if done:
                print("thisorder",action_this_epi)
                print("thisreward", reward.item())
                print("thislatency", reward.item()*pg_cost)
                #             print("done")
                next_value = 0
                sqlt.updateBestOrder(reward.item(),action_this_epi)
                print("bestorder",sqlt.getBestOrder())
                print("bestreward", sqlt.bestLatency)
                print("bestlantency", sqlt.bestLatency*pg_cost)
            ...
            if done:
                ...
                if ((i_episode + 1)%print_every==0):
                    ...
                    # print("train")
                    # val_value = DQN.validate(trainSet)
                    # print("test")
                    ...
                    if val_value < best_val_value:
                        ...
                        best_epoch = i_episode // print_every
                        torch.save(policy_net.cpu().state_dict(), 'bestLatencyTuning.pth')
                        print(f"Best model saved at Epoch: {best_epoch}, val_value: {best_val_value}")
                        policy_net.to(device)
                        print("~~~~~~~~~~~~~~")
                    # logger.info("loss: %s", np.mean(losses))
                    # logger.info("######################Epoch %s %s", i_episode // print_every, pg_cost)
                    # val_value = DQN.validate(validateSet)
                    # logger.info("time %s", time.time() - startTime)
                    # logger.info("~~~~~~~~~~~~~~")
                break
...
if __name__=='__main__':
    ...
    # Q4,Q1=split_queries_by_filename(JOBQueries)
    print(len(Q4),len(Q1))
    # logger.info("%s %s", len(Q4), len(Q1))
